    ...
    const char *fmt, va_list args)
{
    ...
    while (*fmt && buf < pend) 
    {
        if (*fmt == '%') 
        {
            ...
            else if (ui64 <= SHS_MAX_UINT32_VALUE) 
            {
                ui32 = (uint32_t) ui64;
                do 
                {
    ...
    size_t size, uint32_t type)
{
    ...
    while (size--) 
    {
        ...
        case sw_usual:
            if (ch == '?'
                && (type & (SHS_UNESCAPE_URI|SHS_UNESCAPE_REDIRECT))) 
            {
                *d++ = ch;
