 ...
 * keys. 
 */
extern void gl_3des_set3keys (gl_3des_ctx *ctx, const char * key1, 
 ...
 */
extern bool gl_3des_makekey (gl_3des_ctx *ctx, const char * key,
    size_t keylen);
 ...
 */
extern void gl_3des_ecb_crypt (gl_3des_ctx *ctx, const char * from,
    char * to, int mode);
...
#define SHS_MATH_DFSLOG2_DOWN (0)
#define SHS_MATH_DFSLOG2_UP   (1)

#define SHS_MATH_ROUND_UP(x, alignment)  \
    (((x) + (alignment) - 1) & ~((alignment) - 1))

#define SHS_MATH_ALIGN_SIZE       sizeof(void *)
#define SHS_MATH_ALIGNMENT(x, align) \
    (((x) % (align)) ? ((x) + ((align) - (x) % (align))) : (x))

unsigned int shs_math_dfslog2(size_t x, int round);
int          shs_math_is_prime(size_t x);
size_t       shs_math_find_prime(size_t x);
